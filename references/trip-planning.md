...
触发技能后，先核对以下五类信息。五类全部明确后才能检查接口、调研和排行程：

1. 旅游城市；
2. 入住酒店名称和地址；
3. 抵达日期、本地时间、机场/车站；
4. 离开日期、本地时间、机场/车站；
5. 想去或偏好的景点，以及想吃或偏好的美食/菜系。

图片里的信息必须先提取并回显。偏好可以写“请你代选”，但城市、酒店和抵离信息不能猜。任一项缺失时，一次列出缺项并停在本门禁，不提前查店或排行程。
...
把完整文字版交给用户修改。只有用户明确确认“文字方案最终版”后，才进入网页制作；任何修改都会回到本步骤重新给出完整版本并再次确认。
...
Options:
  A) 文字方案无误，生成手机端高德地图页面
  B) 我要调几个地方
  C) 大方向不对，重新来
```

没有收到 A 或同等明确确认时，不得开始写网页。
